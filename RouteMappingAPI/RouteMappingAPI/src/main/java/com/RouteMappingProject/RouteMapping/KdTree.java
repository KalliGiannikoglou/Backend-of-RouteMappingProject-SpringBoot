package com.RouteMappingProject.RouteMapping;//package com.jwetherell.algorithms.data_structures;
...
public class KdTree<T extends KdTree.XYZPoint> implements Iterable<T> {
     ...
     *
     * @param other
     *            The other KdTree to copy.
     */
    public KdTree(KdTree<T> other) {
        if (other.root != null) {
            this.root = copyNode(other.root, null);
        }
    }
     ...
     *            The node to copy.
     * @param parent
     *            The parent node of @param node
     */
    private KdNode copyNode(KdNode node, KdNode parent) {
        if (node == null) {
            return null;
        }

        KdNode newNode = new KdNode(node.id, node.k, node.depth);
        newNode.parent = parent;
        newNode.lesser = copyNode(node.lesser, newNode);
        newNode.greater = copyNode(node.greater, newNode);
        return newNode;
    }
